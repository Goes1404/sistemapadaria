...
export default function PDV() {
  ...
  return (
    <div className="flex h-screen flex-col bg-stone-100 lg:flex-row">
      {/* Catálogo */}
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex items-center gap-3 border-b border-stone-200 bg-white px-5 py-3.5">
          <button onClick={() => navigate('/app')} className="text-sm text-stone-400 hover:text-stone-700">←</button>
          <div className="flex-1">
            <p className="text-sm font-bold text-stone-900">Frente de caixa</p>
            <p className="text-xs text-stone-500">{caixa.operador} · troco inicial {brl(caixa.trocoInicial)}</p>
          </div>
          <button className="btn-ghost px-3 py-2 text-xs" onClick={() => setMovimentando(true)}>
            Sangria / suprimento
          </button>
          <button className="btn-ghost px-3 py-2 text-xs" onClick={() => { setPagando(false); fecharCaixa() }}>
            Fechar caixa
          </button>
        </header>
        ...
        <div className="grid flex-1 auto-rows-min grid-cols-2 gap-3 overflow-y-auto p-5 sm:grid-cols-3 xl:grid-cols-4">
          {filtrados.map((p) => (
            <button key={p.id} onClick={() => adicionar(p.id)}
              ...
              <span className="text-sm font-semibold leading-snug text-stone-800">{p.nome}</span>
              <span className="mt-3 text-lg font-bold tabular-nums text-crosta-700">
                {brl(p.preco)}{p.porPeso && <span className="text-xs font-normal text-stone-400">/kg</span>}
              </span>
            </button>
          ))}
          {filtrados.length === 0 && (
            <p className="col-span-full py-10 text-center text-sm text-stone-400">Nenhum produto encontrado.</p>
          )}
        </div>
      </div>
      ...
      <aside className="flex w-full shrink-0 flex-col border-t border-stone-200 bg-white lg:w-96 lg:border-l lg:border-t-0">
        <header className="border-b border-stone-200 px-5 py-3.5">
          <p className="text-sm font-bold text-stone-900">Venda atual</p>
          <p className="text-xs text-stone-500">{itens.length} {itens.length === 1 ? 'item' : 'itens'}</p>
        </header>
              ...
              {itens.map((i) => (
                <li key={i.produtoId} className="flex items-center gap-3 px-5 py-3">
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-stone-800">{i.nome}</p>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => alterarQuantidade(i.produtoId, -1)}
                            className="h-7 w-7 rounded-md bg-stone-100 text-stone-600 hover:bg-stone-200">−</button>
                    <button onClick={() => alterarQuantidade(i.produtoId, 1)}
                            className="h-7 w-7 rounded-md bg-stone-100 text-stone-600 hover:bg-stone-200">+</button>
                  </div>
        ...
        <footer className="border-t border-stone-200 p-5">
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <span className="text-3xl font-bold tabular-nums text-stone-900">{brl(total)}</span>
          </div>
...
function ModalPeso({ item, onFechar, onConfirmar }: {
  ...
  return (
    <Caixa>
      ...
      <p className="mt-0.5 text-sm text-stone-500">{brl(item.preco)} por kg — informe o peso da balança.</p>
      ...
      <p className="mt-3 text-center text-sm text-stone-600">
        Total do item: <strong className="text-lg tabular-nums text-stone-900">{brl(valor)}</strong>
      </p>
...
function ModalPagamento({ total, onFechar, onConfirmar }: {
  ...
  return (
    <Caixa>
      <h2 className="text-lg font-bold text-stone-900">Pagamento</h2>
      <p className="mt-0.5 text-sm text-stone-500">Aceita divisão entre várias formas no mesmo pedido.</p>

      <div className="mt-5 rounded-lg bg-stone-100 p-4">
        <div className="flex justify-between text-sm">
          <span className="text-stone-600">Total da venda</span>
          <span className="font-semibold tabular-nums">{brl(total)}</span>
        </div>
        <div className="mt-1.5 flex justify-between text-sm">
          <span className="text-stone-600">Já informado</span>
          <span className="font-semibold tabular-nums text-emerald-700">{brl(pago)}</span>
        </div>
        <div className="mt-2 flex justify-between border-t border-stone-200 pt-2">
          <span className="font-semibold text-stone-800">{troco > 0 ? 'Troco' : 'Falta'}</span>
          <span className={`text-xl font-bold tabular-nums ${troco > 0 ? 'text-sky-700' : 'text-stone-900'}`}>
      ...
      {pagamentos.length > 0 && (
        <ul className="mt-4 space-y-1.5">
          {pagamentos.map((p, i) => (
            <li key={i} className="flex items-center justify-between rounded-md bg-white px-3 py-2 text-sm ring-1 ring-stone-200">
              <span className="text-stone-700">{formas.find((f) => f.id === p.forma)?.rotulo}</span>
            ...
            {formas.map((f) => (
              <button key={f.id} onClick={() => setForma(f.id)}
                className={`rounded-lg px-2 py-2.5 text-xs font-semibold transition-colors ${
                  forma === f.id ? 'bg-crosta-600 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
                {f.rotulo}
              </button>
            ))}
          </div>
  ...
  return (
    <Caixa>
      <div className="text-center">
        ...
        <h2 className="mt-2 text-lg font-bold text-stone-900">Venda finalizada</h2>
        <p className="mt-4 text-3xl font-bold tabular-nums text-stone-900">{brl(dados.total)}</p>
...
function ModalMovimento({ onFechar, onConfirmar, resumo }: {
  ...
  return (
    <Caixa>
      <h2 className="text-lg font-bold text-stone-900">Movimentar caixa</h2>
        ...
        {(['SANGRIA', 'SUPRIMENTO'] as const).map((m) => (
          <button key={m} onClick={() => setMotivo(m)}
            className={`rounded-lg px-3 py-2.5 text-sm font-semibold transition-colors ${
              motivo === m ? 'bg-crosta-600 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-stone-100 p-6">
      <div className="w-full max-w-sm">
        <button onClick={onVoltar} className="mb-4 text-sm text-stone-400 hover:text-stone-700">← backoffice</button>
        <div className="card card-pad">
          <h1 className="text-xl font-bold text-stone-900">Abertura de caixa</h1>
